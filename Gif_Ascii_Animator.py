...
def convert_image_to_ascii(image, scale=1.0):
    ...
    imgx = int(imgx / chrx * scale)
    imgy = int(imgy / chry * scale)
...
def convert_frames_to_ascii(frames: list, scale = 1.0):
    ascii_frames = []
    for frame in frames:
        new_frame = convert_image_to_ascii(frame, scale)
...
def animate_ascii(ascii_frames, frame_pause=.02, num_iterations=15, clear_prev_frame=False):
...
if __name__ == "__main__":
    im = Image.open("horse-plinko.gif")
